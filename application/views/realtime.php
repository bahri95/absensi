...
$this->load->helper('url');
  ...
  echo "<script> var lati=[];var longi=[];var a=0;var nama=[];var foto=[];</script>";
foreach ($pergerakan as $gerak) {
 echo '<script>    
    foto[a]="'.$gerak['foto'].'"; nama[a]="'.$gerak['nama'].'";lati[a]='.$gerak['lat'].';longi[a]='.$gerak['longi'].';a++;</script>';
}
...
<h1 style="text-align:center">Berikut Posisi Terakhir Pegawai</h1>
<hr>
...
if(besar==0){
 ...
 function initMap() {
  var icon = {
    ...
    map:map,
    position: new google.maps.LatLng(lati[0], longi[0]),
    title: nama[0],
     icon: icon
  });
      }
