										...
										if ($uploadOk == 0) {
														    ...
														    if (move_uploaded_file($_FILES['pic']["tmp_name"], $target_file)) {
														       ...
														       $Durations = array(
																...
																$this->dauo->input_data($Durations,'datatenaga');
																	echo '<script>alert("Data Terimpan")</script>';
																	echo '<script>document.location.href= "'.base_url('Preupdating').'"</script>';
														    } else {
																	echo '<script>alert("Data Gagal Terimpan")</script>';
																	echo '<script>document.location.href= "'.base_url('Tambah').'"</script>';
														    }
														}
				 		}else{
													 			echo "please upload an image";
													 			echo'<script>  setTimeout(function () {
															       window.location.href = "'.base_url().'"; //will redirect to your Main page (an ex: blog.html)
															    }, 7000); //will call the function after 2 secs.</script>';
				 		}
	}
}
